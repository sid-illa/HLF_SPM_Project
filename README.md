...
## then extract the archive you just downloaded into /usr/local, creating a fresh Go tree in /usr/local/go: Use sudo
...
# We have built this software based on the Fabric Samples Test network
## Download the fabric samples into the specified location as shown below
...
cd DBScripts
run the Scripts.sql file in PostgresSQL DB
...
## Then run the following applications to enroll the admin user for a organization, and register new users which will be used by the other applications
